...
public class TypeEnvironment implements Environment<Symbol, ExprType> {
  ...
  /**
   * Resolve the {@link Symbol} from environment.
   *
   * @param symbol Symbol
  ...
  public ExprType resolve(Symbol symbol) {
    ...
    throw new SemanticCheckException(
        String.format("can't resolve expression %s in type env", symbol));
  }
  ...
  public void define(Expression var, ExprType type) {
    if (var instanceof ReferenceExpression) {
      ...
      define(new Symbol(Namespace.FIELD_NAME, ref.getAttr()), type);
    } else {
