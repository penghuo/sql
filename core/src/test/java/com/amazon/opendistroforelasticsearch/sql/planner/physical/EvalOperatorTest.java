...
class EvalOperatorTest extends PhysicalPlanTestBase {
  ...
  public void create_multiple_field_using_field_defined_in_input_tuple() {
    ...
    PhysicalPlan plan =
        eval(
            inputPlan,
            ImmutablePair.of(
                DSL.ref("velocity", INTEGER), dsl.divide(DSL.ref("distance", INTEGER), DSL.ref("time", INTEGER))),
            ImmutablePair.of(
  ...
  public void do_nothing_with_none_tuple_value() {
    ...
    PhysicalPlan plan = eval(inputPlan, ImmutablePair.of(DSL.ref("response", INTEGER), DSL.ref(
        "referer", STRING)));
